...
def _build_phase3_dataset(include_znw: bool = True) -> xr.Dataset:
...
def test_phase3_reconstructs_theta_pressure_temperature_and_risk_masks() -> None:
...
def test_phase3_writes_artifacts_and_exports_proxy_fields(tmp_path: Path) -> None:
    dataset = _build_phase3_dataset()
    product = build_phase3_approximate_risk_product(dataset, "synthetic.nc", time_index=0)
    ...
    assert written.attrs["risk_definition"].startswith("risk_presence_3d = liquid_presence_3d")
    assert float(written["risk_presence"].sum()) == 1.0


def test_phase3_requires_znw_for_eta_proxy() -> None:
    dataset = _build_phase3_dataset(include_znw=False)

    with pytest.raises(ValueError, match="ZNW"):
        build_phase3_approximate_risk_product(dataset, "synthetic.nc", time_index=0)


def test_phase3_pressure_assumption_matches_config_constants() -> None:
