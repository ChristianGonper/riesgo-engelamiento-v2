...
from riesgo_engelamiento.phase6 import build_phase6_heuristic_severity_product, write_phase6_outputs


def _build_phase6_dataset() -> xr.Dataset:
...
def test_phase6_builds_band_summary_and_temporal_severity() -> None:
    dataset = _build_phase6_dataset()
    product = build_phase6_heuristic_severity_product(dataset, "synthetic.nc", time_index=0)
    ...
    phase6_markdown = product.to_markdown(phase_number=6)
    phase6_dict = product.to_dict(phase_number=6)
    assert phase6_dict["phase"] == 6
    assert phase6_dict["product_kind"] == "heuristic proxy"
    assert "# Fase 6: severidad heuristica y rangos relativos del modelo" in phase6_markdown
    assert "Relative bands" in phase6_markdown
    assert "Temporal severity" in phase6_markdown


def test_phase6_writes_artifacts_and_exports_summary_fields(tmp_path: Path) -> None:
    dataset = _build_phase6_dataset()
    product = build_phase6_heuristic_severity_product(dataset, "synthetic.nc", time_index=0)
    markdown_path, json_path, netcdf_path, png_path = write_phase6_outputs(product, tmp_path)
    ...
    assert written.attrs["title"].startswith("Phase 6 heuristic severity diagnostic")
    assert written.attrs["product_kind"] == "heuristic proxy"
    assert written.attrs["phase"] == 6
    assert "eta_mid" in written.coords


def test_phase6_requires_qice() -> None:
    dataset = _build_phase6_dataset().drop_vars("QICE")

    with pytest.raises(ValueError, match="QICE"):
        build_phase6_heuristic_severity_product(dataset, "synthetic.nc", time_index=0)


def test_phase6_score_for_previous_time_is_not_changed_by_future_extension() -> None:
    prefix = _build_phase6_dataset()
    ...
    prefix_product = build_phase6_heuristic_severity_product(prefix, "synthetic.nc", time_index=1)
    extended_product = build_phase6_heuristic_severity_product(extended, "synthetic.nc", time_index=1)
