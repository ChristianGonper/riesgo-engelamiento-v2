...
Primero, se añadirá una visualización cartográfica de mayor calidad para el dominio WRF. En lugar de mostrar únicamente una máscara binaria o un panel técnico, el sistema deberá generar mapas con contexto geográfico visible, una simbología más informativa y una composición visual apta para incluirse en una entrega o presentación. El mapa deberá representar, como mínimo, el riesgo aproximado o la severidad heurística para un tiempo seleccionado y, cuando proceda, para una capa vertical o banda relativa concreta.
...
6. Como profesor, quiero que la leyenda y la simbología indiquen claramente qué significa cada color o categoría, para no depender del texto técnico del código.
7. Como usuario, quiero ver no solo presencia o ausencia, sino una representación del nivel de riesgo o severidad heurística, para entender mejor la distribución espacial del fenómeno.
8. Como usuario, quiero seleccionar un tiempo concreto para generar el mapa final, para centrar el análisis en un caso representativo.
9. Como usuario, quiero poder generar mapas para varios tiempos destacados, para comparar la evolución del caso.
10. Como usuario, quiero que el mapa final pueda representar la severidad total del dominio o una banda vertical concreta, para responder preguntas más específicas.
11. Como usuario, quiero elegir una banda vertical relativa de interés, para inspeccionar el riesgo en capas bajas, medias o altas del modelo.
12. Como usuario, quiero que el sistema indique qué banda vertical relativa domina el riesgo en cada instante, para interpretar rápidamente la estructura vertical.
13. Como usuario, quiero una tabla o resumen que acompañe al mapa, para ver clase de severidad, cobertura espacial y persistencia sin abrir varios archivos distintos.
14. Como usuario, quiero que el producto final explique explícitamente si está mostrando una máscara binaria, un proxy aproximado o una severidad heurística, para no sobreinterpretar la figura.
15. Como usuario, quiero poder distinguir entre un mapa del riesgo aproximado y un mapa de la severidad heurística, para elegir la salida que mejor responda a mi pregunta.
16. Como usuario, quiero una forma de centrar la interpretación en aeronaves, para traducir el resultado meteorológico a una lectura más aplicada.
17. Como usuario, quiero que el sistema identifique rangos verticales relativos de mayor interés para vuelo, para saber en qué capa del modelo conviene fijarse.
18. Como usuario, quiero que las bandas verticales se describan en un lenguaje legible, para comunicar mejor los resultados aunque no disponga de altura geométrica exacta.
19. Como usuario, quiero que el producto final resuma si el principal riesgo está en banda baja, media o alta, para poder comunicar la conclusión de forma compacta.
20. Como usuario, quiero que el mapa destaque visualmente las áreas de mayor riesgo, para poder localizarlas de un vistazo.
21. Como usuario, quiero que los colores usados transmitan gradación de peligro y no solo categorías binarias, para que la figura sea más informativa.
22. Como usuario, quiero que la salida pueda incluir anotaciones o recuadros de resumen, para que el PNG por sí solo sea más autosuficiente.
23. Como usuario, quiero que el sistema siga exportando formatos máquina-legibles, para poder reutilizar la información en análisis posteriores.
24. Como desarrollador, quiero reutilizar los productos de Phase 5 y Phase 6 como entradas de una capa de visualización final, para no duplicar la física.
25. Como desarrollador, quiero separar claramente el cálculo del riesgo de la composición cartográfica, para mantener el código testeable.
26. Como desarrollador, quiero centralizar paletas, estilos, umbrales visuales y configuración de layout, para poder iterar sobre el diseño sin tocar la lógica científica.
27. Como desarrollador, quiero una interfaz simple para generar mapas finales por tiempo y por banda vertical, para que la CLI siga siendo reproducible.
28. Como desarrollador, quiero introducir un concepto explícito de "producto final" o "vista final", para no seguir mezclando artefactos técnicos y artefactos de presentación.
29. Como desarrollador, quiero que los resúmenes de salida incluyan metadatos suficientes para reconstruir la figura o justificarla, para facilitar validación y trazabilidad.
30. Como desarrollador, quiero pruebas que verifiquen el contenido semántico de los mapas y resúmenes, para evitar regresiones cuando cambie la presentación.
31. Como investigador, quiero una vista espacial de la severidad heurística por dominio, para estudiar no solo cobertura sino intensidad relativa.
32. Como investigador, quiero una vista espacial condicionada por banda vertical, para analizar cómo cambia el riesgo según la estructura relativa del perfil.
33. Como investigador, quiero comparar el mapa del riesgo aproximado con el de severidad heurística, para entender qué añade la heurística.
34. Como investigador, quiero conservar la serie temporal y la estructura vertical actuales como respaldo, para no perder la interpretación diagnóstica detrás del producto final.
35. Como docente, quiero que el trabajo final deje claras las limitaciones de altitud y termodinámica, para que el resultado sea defendible académicamente.
36. Como docente, quiero una salida visual que pueda insertarse casi directamente en una memoria o presentación, para valorar el acabado final del proyecto.
37. Como docente, quiero que la representación final responda mejor a la pregunta práctica de "dónde" y "en qué capa" está el riesgo, para que el ejercicio no se quede en una máscara técnica.
38. Como usuario, quiero que la CLI permita pedir explícitamente la figura final, para no tener que recombinar manualmente artefactos intermedios.
39. Como usuario, quiero una nomenclatura clara entre productos diagnósticos y productos finales, para entender qué archivo debo abrir primero.
40. Como usuario, quiero que la salida final destaque los tiempos más relevantes de la serie, para centrar la interpretación en los momentos de mayor interés.
...
- La visualización final deberá incluir contexto geográfico visible del dominio, en la medida permitida por las librerías y dependencias adoptadas.
...
- Si se añaden dependencias cartográficas, se adoptarán solo si aportan un salto claro de calidad visual y siguen siendo reproducibles en el entorno del proyecto.
